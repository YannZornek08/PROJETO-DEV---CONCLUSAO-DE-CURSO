...
class AllItemAdditionalController {
  async handle(req: Request, res: Response) {
    const { category_id, order_id } = req.body; // ou req.params.product_id se a rota for /:product_id

    const allItemAdditionalService = new AllItemAdditionalService();

    try {
      const result = await allItemAdditionalService.execute({ category_id }, { order_id });
      res.json(result);
    } catch (err: any) {
